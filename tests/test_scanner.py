...
class TestScanner(unittest.TestCase) :
    ...
    def test_gridconnectobserver(self):
        ...
        assert gc_end_byte_idx == len(bytes) - 2, "test is flawed"
        scanner = GridConnectObserver()
        self.assertEqual(scanner._delimiter, GC_END_BYTE)  # subclass' default
        self.assertFalse(scanner.hasNext())
        self.assertFalse(scanner.hasNextByte())
        scanner.push(bytes[0])
        ...
        self.assertFalse(scanner.hasNext())
        self.assertTrue(scanner.hasNextByte())  # make sure not mutated
        assert gc_end_byte_idx < len(bytes)-1, "test is flawed"
        ...
        self.assertFalse(scanner.hasNextByte())
        self.assertEqual(len(scanner._buffer), 0)  # we didn't yet add "\n"
        scanner.push(bytes[-1])  # finally add the "\n", and this also makes
        #   sure int append is allowed though += (bytes or bytearray) is
        #   the most common usage (bytearray and bytes are equivalent to
        #   list[int] merely with further constraints, so allow int arg).
        self.assertEqual(len(scanner._buffer), 1)  # should have 1 left ("\n")
